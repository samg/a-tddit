class Baz23sController < ApplicationController
  before_action :set_baz23, only: %i[ show edit update destroy ]

  # GET /baz23s or /baz23s.json
  def index
    @baz23s = Baz23.all
  end

  # GET /baz23s/1 or /baz23s/1.json
  def show
  end

  # GET /baz23s/new
  def new
    @baz23 = Baz23.new
  end

  # GET /baz23s/1/edit
  def edit
  end

  # POST /baz23s or /baz23s.json
  def create
    @baz23 = Baz23.new(baz23_params)

    respond_to do |format|
      if @baz23.save
        format.html { redirect_to @baz23, notice: "Baz23 was successfully created." }
        format.json { render :show, status: :created, location: @baz23 }
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @baz23.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /baz23s/1 or /baz23s/1.json
  def update
    respond_to do |format|
      if @baz23.update(baz23_params)
        format.html { redirect_to @baz23, notice: "Baz23 was successfully updated." }
        format.json { render :show, status: :ok, location: @baz23 }
      else
        format.html { render :edit, status: :unprocessable_entity }
        format.json { render json: @baz23.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /baz23s/1 or /baz23s/1.json
  def destroy
    @baz23.destroy
    respond_to do |format|
      format.html { redirect_to baz23s_url, notice: "Baz23 was successfully destroyed." }
      format.json { head :no_content }
    end
  end

  private
    # Use callbacks to share common setup or constraints between actions.
    def set_baz23
      @baz23 = Baz23.find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def baz23_params
      params.require(:baz23).permit(:foo1, :foo2, :foo3, :foo4, :foo5)
    end
end
