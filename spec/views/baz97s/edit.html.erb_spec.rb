require 'rails_helper'

RSpec.describe "baz97s/edit", type: :view do
  before(:each) do
    @baz97 = assign(:baz97, Baz97.create!(
      foo1: "MyString",
      foo2: "9.99",
      foo3: 1,
      foo4: false,
      foo5: "MyText"
    ))
  end

  it "renders the edit baz97 form" do
    render

    assert_select "form[action=?][method=?]", baz97_path(@baz97), "post" do

      assert_select "input[name=?]", "baz97[foo1]"

      assert_select "input[name=?]", "baz97[foo2]"

      assert_select "input[name=?]", "baz97[foo3]"

      assert_select "input[name=?]", "baz97[foo4]"

      assert_select "textarea[name=?]", "baz97[foo5]"
    end
  end
end
