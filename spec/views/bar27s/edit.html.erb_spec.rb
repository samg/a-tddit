require 'rails_helper'

RSpec.describe "bar27s/edit", type: :view do
  before(:each) do
    @bar27 = assign(:bar27, Bar27.create!(
      foo1: "MyString",
      foo2: "9.99",
      foo3: 1,
      foo4: false,
      foo5: "MyText"
    ))
  end

  it "renders the edit bar27 form" do
    render

    assert_select "form[action=?][method=?]", bar27_path(@bar27), "post" do

      assert_select "input[name=?]", "bar27[foo1]"

      assert_select "input[name=?]", "bar27[foo2]"

      assert_select "input[name=?]", "bar27[foo3]"

      assert_select "input[name=?]", "bar27[foo4]"

      assert_select "textarea[name=?]", "bar27[foo5]"
    end
  end
end
