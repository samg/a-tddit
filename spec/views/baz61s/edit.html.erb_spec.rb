require 'rails_helper'

RSpec.describe "baz61s/edit", type: :view do
  before(:each) do
    @baz61 = assign(:baz61, Baz61.create!(
      foo1: "MyString",
      foo2: "9.99",
      foo3: 1,
      foo4: false,
      foo5: "MyText"
    ))
  end

  it "renders the edit baz61 form" do
    render

    assert_select "form[action=?][method=?]", baz61_path(@baz61), "post" do

      assert_select "input[name=?]", "baz61[foo1]"

      assert_select "input[name=?]", "baz61[foo2]"

      assert_select "input[name=?]", "baz61[foo3]"

      assert_select "input[name=?]", "baz61[foo4]"

      assert_select "textarea[name=?]", "baz61[foo5]"
    end
  end
end
